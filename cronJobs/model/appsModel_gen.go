...
var (
	...
	appsRowsWithPlaceHolder = strings.Join(stringx.Remove(appsFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), "=?,") + "=?"
)
...
func (m *defaultAppsModel) Insert(ctx context.Context, data *Apps) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.AccountId, data.AppId, data.AppName, data.PkgName, data.Channel, data.AppType, data.Tags, data.CreatedAt, data.UpdatedAt)
	return ret, err
}
...
func (m *defaultAppsModel) Update(ctx context.Context, newData *Apps) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.AccountId, newData.AppId, newData.AppName, newData.PkgName, newData.Channel, newData.AppType, newData.Tags, newData.CreatedAt, newData.UpdatedAt, newData.Id)
	return err
}
