    ...
    <el-row>
      <el-col :span="24" class="search-container">
        <el-form ref="_search" :model="search" inline size="mini">
          <el-form-item>
            <el-input v-model="search.trans_sn" clearable placeholder="流水单号"/>
          </el-form-item>
          <el-form-item>
      ...
      <el-col :span="24" style="margin-bottom: 15px;">
        <el-button icon="el-icon-plus" size="small" type="primary" @click="add" v-permission="'trans-platform/create'">添加流水</el-button>
        <el-button icon="el-icon-download" size="small" @click="download" :loading="loadings.downloadLoading" v-permission="'trans-platform/download'">导出</el-button>
      </el-col>
      <el-col :span="24">
        <el-table v-loading="loadings.pageLoading" :data="transList.list" highlight-current-row stripe border
          ...
          <el-table-column prop="trans_sn" label="流水编号" width="130" align="center"/>
          <el-table-column prop="from_account_name" label="广告平台转出账号" width="160" show-overflow-tooltip/>
          <el-table-column prop="to_account_name" label="创梦转入账号" width="160" show-overflow-tooltip/>
          ...
          <el-table-column label="状态" width="80" align="center">
            <template slot-scope="scope">
              <span :class="scope.row.state|stateClassFilter">{{transList.flow_state[scope.row.state]}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="profit_quarter" label="返点归属" width="80" align="center"/>
          <el-table-column label="操作" width="100" align="center">
            <template slot-scope="scope">
              <el-button-group class="table-operate" v-if="scope.row.state > 0">
                <el-button type="primary" plain @click.native.prevent="editRow(scope.row)" v-permission="'trans-platform/update'">修改</el-button>
                <el-button type="danger" plain @click.native.prevent="destroyRow(scope.row)" v-permission="'trans-platform/destroy'">作废</el-button>
              </el-button-group>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-col :span="24" style="margin-top: 15px;">
        <page :page="search.page" :total="transList.total" @current-change="handlePage" @size-change="handlePageSize"/>
      </el-col>
    </el-row>

    <trans-platform-create ref="create" @success="getTransList"/>
    <rebate-quarter-update ref="update" @success="getTransList"/>
  </div>
</template>

<script>
  import {transPlatformList, transPlatformListDownload, transPlatformDestroy} from '@a/trans'
  import Page from '@c/Page'
  import TransPlatformCreate from './components/trans-platform-add'
  ...
  export default {
    ...
    components: {
      Page, TransPlatformCreate, RebateQuarterUpdate
    },
    data() {
      return {
        loadings: {
        ...
        search: {
          ...
          order_type: '', order_field: '', page: 1, limit: 10
        },
      }
    },
    mounted() {
    ...
    methods: {
      getTransList() {
        ...
        transPlatformList(this.search).then(response => {
      ...
      download() {
        ...
        transPlatformListDownload(this.search).then(res => {
          ...
          import('@/vendor/Export2Excel').then(excel => {
            ...
            excel.export_json_to_excel({header: tHeader, data, filename: "广告平台转账记录"})
            this.loadings.downloadLoading = false
          }).catch(err => {
            console.log(err)
            this.loadings.downloadLoading = false
      ...
      destroyRow(row) {
        this.$confirm('确认作废此流水么?', '警告', {
          ...
          this.loadings.pageLoading = true
          transPlatformDestroy(row.id).then(() => {
