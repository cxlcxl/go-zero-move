<template>
  <div v-loading="loading" class="report-container">
    ...
    <el-tabs v-model="activeTopic" type="border-card" @tab-click="handleChangeTopic">
      <el-tab-pane v-for="(title, topic) in rechargeList.topics" :label="title" :name="topic" />
      <line-chart v-if="renderChart" ref="line" :chart-data="lineChartData" :x-axis="search.month_range" />
    </el-tabs>

    <data-table ref="data" style="margin-top: 10px;" :data-list="rechargeList" :summary-columns="summaryColumns" />
  </div>
</template>
...
import { rechargeReport } from '@a/report'
  ...
  methods: {
    getReportData() {
      ...
      Promise.all([
        ...
        rechargeReport(this.search).then(res => {
          if (res.data.topics) {
    ...
    // 重新渲染图表
    reRenderChart(topic) {
      const chartData = formatLineChart(topic, this.rechargeList.list, this.search.month_range, this.search.has_customer)
    ...
    output() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        ...
        this.rechargeList.columns.map(item => {
          tHeader.push(item.title)
          tmpHeaders.push({ code: item.field })
        })
        const data = excel.formatJson({ headers: tmpHeaders, list: this.rechargeList.list })
        if (data.length === 0) {
          this.$message.info('没有筛选到需导出的数据')
          this.loading = false
          return
        }
        excel.export_json_to_excel({ header: tHeader, data, filename: '客户充值报表数据' })
        this.loading = false
      }).catch(err => {
        this.loading = false
      })
